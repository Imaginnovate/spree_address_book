Spree::Api::V1::CheckoutsController.class_eval do
  ...
  def set_addresses
    ...
    if params[:order][:ship_address_id].to_i > 0
      ...
      Spree::Address.find(params[:order][:ship_address_id]).user_id != current_api_user.id && raise("Frontend address forging")
    else
    ...
    if params[:order][:bill_address_id].to_i > 0
      ...
      Spree::Address.find(params[:order][:bill_address_id]).user_id != current_api_user.id && raise("Frontend address forging")
    else
  ...
  def normalize_addresses
    ...
    if @order.bill_address_id != @order.ship_address_id && bill_address.same_as?(ship_address)
      ...
      bill_address.update_attribute(:user_id, current_api_user.try(:id))
    end

    ship_address.update_attribute(:user_id, current_api_user.try(:id))
  end
end